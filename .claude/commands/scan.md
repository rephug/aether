...
perfection: `/enrich` handles quality later. `batch-size` (default 100) caps how many
symbols this session processes before stopping.

## Procedure

1. Find the targets. Call `aether_audit_candidates` with `crate_filter: "<crate>"` and a
   generous `top_n` (the batch size). Mock and low-confidence SIRs rank first because the
   mock provider writes them at confidence 0.1. Keep only candidates whose
   `current_confidence` is below 0.2 or whose SIR intent starts with `[MOCK]`.
   If the tool is unavailable, the equivalent SQLite pre-filter is:
   `SELECT id FROM sir WHERE json_extract(sir_json, '$.confidence') < 0.2 OR sir_json LIKE '[MOCK]%'`
   against `.aether/meta.sqlite`, joined to `symbols` on `id` and filtered by
   `file_path LIKE 'crates/<crate>/%'`.
2. Group the candidate symbols by source file.
...
5. Stop after `batch-size` symbols and print how many remain (rerun `/scan` or let

...
mod tests {
    use super::{
        FileCommunityConfig, FileSymbol, SymbolEntry, WeightedGraph, apply_container_rescue,
        apply_semantic_rescue, build_anchor_groups, build_rep_members, confidence_label,
        detect_file_communities, merge_small_communities, qualified_name_stem,
    };
    use aether_core::SymbolKind;
    ...
    fn ablation_aether_store() {
        print_ablation_placeholder("aether-store");
    }

    #[test]
    #[ignore]
    fn ablation_aether_config() {
        print_ablation_placeholder("aether-config");
    }

    #[test]
    #[ignore]
    fn ablation_aether_mcp() {
        print_ablation_placeholder("aether-mcp");
    }

    fn print_ablation_placeholder(crate_name: &str) {
        let workspace = std::path::Path::new("/home/rephu/projects/aether/.aether");
        if !workspace.exists() {
            eprintln!(
        ...
        for label in [
            ...
            println!(
                "{:<28} {:<12} {:<12} {:<12} {:<12} {:<10} {:<10} {}",
                label, "manual", "manual", "manual", "manual", "manual", "manual", crate_name
            );
        }
    }
}

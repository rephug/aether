    ...
    support::html_markup_response(html! {
        div class="space-y-4" data-page="architecture" data-granularity=(granularity.clone()) {
            div class="flex items-center justify-between gap-3" {
                h2 class="text-lg font-semibold" { "Architecture Map" }
            ...
            div class="flex flex-wrap gap-2 text-xs" {
                span id="architecture-community-count" class="badge badge-muted" { "Communities: —" }
                span id="architecture-misplaced-count" class="badge badge-orange" { "Misplaced: —" }

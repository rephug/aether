...
mod tests {
    ...
    #[test]
    fn unix_timestamp_secs_logs_on_clock_error() {
        let error = UNIX_EPOCH
            .duration_since(std::time::SystemTime::now())
            .expect_err("clock error");

        let (timestamp, logs) = capture_logs(|| unix_timestamp_secs(Err(error)));

        assert_eq!(timestamp, 0);
        assert!(logs.contains("system clock before Unix epoch, using 0"));
    }
}

# AETHER Dashboard Revision — Codex Prompts v3, Part 3 of 3

**Phases covered:** G (LLM Suite Part 1: Difficulty Radar + Prompt Builder + Context Advisor), H (LLM Suite Part 2: Decomposer + Checkpoints + Autopsy)
**Date:** March 2026
**Companion files:**
...
## Phase G: LLM Collaboration Suite Part 1 — Difficulty Radar, Prompt Builder, Context Advisor
...
PREREQUISITES:
- Phase C (Anatomy — layer data, narrative module)
- Phase D (Glossary — difficulty column display)
- Phase E (Symbol Deep Dive — difficulty section)
Must all be merged to main.

TECHNOLOGY: HTMX + D3.js + Tailwind CSS (all from CDN). Maud for fragments.

=== CONTEXT ===

This phase begins the LLM collaboration teaching suite — features that teach
users how to work effectively with AI coding agents by using their own
codebase as the textbook. The key insight: AETHER already knows what makes
code complex (error modes, side effects, async patterns, dependency depth).
That same data tells us where LLMs will struggle and what prompting strategies
will succeed.

IMPORTANT: ALL output is template-composed from existing SIR data, dependency
...
=== FEATURE 1: LLM DIFFICULTY RADAR ===

PURPOSE: Score every component on how hard it would be for an LLM to generate
correctly, and explain WHY. This helps users prioritize where to invest
...
1. GLOSSARY (Phase D): Add "Difficulty" column/badge to each term card.
   Show emoji + label. Hover tooltip shows reasons.

2. SYMBOL DEEP DIVE (Phase E): Add "LLM Difficulty" section before the
   bottom action buttons. Show:
   - Emoji + label (large)
   - Guidance sentence
   - Bulleted reasons list
   - "See the Prompt Advisor for guidance →" link (Phase H)
...
=== FEATURE 2: ENHANCED PROMPT BUILDER + CODE-TO-SPEC ===
...
Enable "📋 Spec" buttons in Glossary (Phase D left them disabled).
...
=== FEATURE 3: CONTEXT WINDOW ADVISOR ===
...
selection is a skill — too much drowns the signal, too little leaves the
LLM guessing.
...
## Phase H: LLM Collaboration Suite Part 2 — Decomposer, Checkpoints, Autopsy
...
PREREQUISITES: Phases C, D, E, G must all be merged to main.
TECHNOLOGY: HTMX + D3.js + Tailwind CSS (all from CDN). Maud for fragments.

=== CONTEXT ===

This is the capstone phase. The Prompt Decomposer, Verification Checkpoints,
and Prompt Autopsy together teach users HOW to collaborate effectively with
LLMs — using real code they can inspect and verify. These features are the
most novel aspects of AETHER's dashboard and differentiate it from every
other code intelligence tool.

CRITICAL: All output is template-composed from existing data. No LLM calls.
The "teaching" content is generated from data patterns in SIR and the
dependency graph. The quality comes from good templates and smart data
selection, not from asking an LLM to write teaching material.

=== FEATURE 1: PROMPT DECOMPOSER — "Build This in N Steps" ===
...
For a given symbol (or file — see below), generate an ordered sequence of
...
=== FEATURE 2: VERIFICATION CHECKPOINTS ===
...
=== API ENDPOINT: GET /api/v1/checkpoints/{symbol} ===

Returns checkpoints for each decomposer step. This is returned ALONGSIDE
the decomposer response (add a "checkpoints" field to each step) rather
than as a separate endpoint. But also support standalone access.

Add to each decomposer step:
...
"checkpoints": [
  ...
  {
    "check": "Arc wraps SharedState, not State directly",
    "why": "The background task handle must be shared. Wrapping State alone loses the task handle.",
    "source": "sir (side_effect: spawns background task)",
    "severity": "critical"
  },
  {
    "check": "State is not pub",
    "why": "Internal implementation detail — only Db should be the public interface. Making State public breaks encapsulation.",
    "source": "sir (no external dependents reference State directly)",
    "severity": "warning"
  }
]
...
=== FEATURE 3: PROMPT AUTOPSY — "What Would Work and What Wouldn't" ===
...
No new sidebar entries. The LLM teaching tools are accessed from:
- Symbol Deep Dive (bottom action buttons)
- Glossary (term action buttons)
- Prompt Builder page (added as tab: "🎓 Learn" alongside "Build" and "Spec")
...
- Difficulty overview (from Phase G)
...
DECOMPOSER:
1. /dashboard/frag/decompose/Db shows 4-5 steps
2. Steps ordered by dependency (foundation → wrapper → operations → cleanup)
3. Each step has a concrete, copy-paste-ready prompt
4. "Why this order" explains the dependency reasoning
5. Context needed lists only previous steps
6. Steps for simple symbols (Frame, Entry) have 1-2 steps
7. Steps for complex symbols (Db, Subscribe) have 4-5 steps

CHECKPOINTS:
8. Each decomposer step has verification checkpoints
9. Checkpoints are specific (not generic "does it compile")
10. Critical checks reference actual dependents ("8 components clone this")
11. Checkbox UI works (client-side, no persistence needed)

AUTOPSY:
12. /dashboard/frag/autopsy/Subscribe shows three prompt levels
13. Good prompt includes types, control flow, edge cases
14. Partial prompt is recognizably incomplete (shows what was removed)
15. Bad prompt is genuinely vague
16. Teaching summary identifies a named pattern
17. Pattern rule is specific and actionable
18. "🎓 Prompt Advisor" buttons in Glossary and Deep Dive are now enabled and functional
19. Prompts page "🎓 Learn" tab shows difficulty overview + navigation

INTEGRATION:
20. All symbol-links throughout G and H features work
21. Navigation between Decomposer, Autopsy, Context Advisor, and Deep Dive
    forms a connected exploration experience
22. Complexity selector from Phase B applies to all new features
...
```
Phase A  Bug Fixes (ship-blocking)                          1 run
Phase B  Plain English Layer                                1 run
Phase C  Project Anatomy + Layer Narratives                 1-2 runs
Phase D  Dynamic Tour + Glossary                            1 run
Phase E  Narrative Engine (Symbol/File Deep Dive + Flow)    2 runs
Phase F  What Changed Recently + Ask AETHER                 1 run
Phase G  LLM Suite 1 (Difficulty + Prompts + Context)       1-2 runs
Phase H  LLM Suite 2 (Decomposer + Checkpoints + Autopsy)  1-2 runs
                                                            --------
                                                            9-12 runs total
```
...
D, E, F independent of each other. G needs D+E. H needs G.

## All New Files Created

| Phase | Files |
|-------|-------|
| C | narrative.rs, api/anatomy.rs, frag/anatomy (+ layer, file sub-frags) |
| D | api/tour.rs, api/glossary.rs, frag/tour, frag/glossary |
| E | api/symbol.rs, api/file.rs, api/flow.rs, frag/symbol, frag/file, frag/flow |
| F | api/changes.rs, api/ask.rs, frag/changes, frag/ask |
| G | api/difficulty.rs, api/prompts.rs, api/spec.rs, api/context.rs, frag/* |
| H | api/decompose.rs, api/autopsy.rs, frag/decompose, frag/autopsy |

## All New API Endpoints

| Endpoint | Phase | Method |
|----------|-------|--------|
| /api/v1/anatomy | C | GET |
| /api/v1/tour | D | GET |
| /api/v1/glossary | D | GET |
| /api/v1/symbol/{name} | E | GET |
| /api/v1/file/{path} | E | GET |
| /api/v1/flow | E | GET |
| /api/v1/changes | F | GET |
| /api/v1/ask | F | POST |
| /api/v1/difficulty | G | GET |
| /api/v1/prompts/search | G | GET |
| /api/v1/prompts/generate | G | GET |
| /api/v1/spec/{symbol} | G | GET |
| /api/v1/context/{symbol} | G | GET |
| /api/v1/decompose/{symbol} | H | GET |
| /api/v1/decompose/file/{path} | H | GET |
| /api/v1/checkpoints/{symbol} | H | GET |
| /api/v1/autopsy/{symbol} | H | GET |

## All New Sidebar Entries (final order)

  📖 Anatomy (C)
  🗺️ Tour (D)
  🔄 Trace Flow (E)
  📚 Glossary (D)
  💬 Prompts (G — tabs: Build | Spec | Context | Learn)
  🕐 Recent Changes (F)
  ─── separator ───
  Overview (existing)
